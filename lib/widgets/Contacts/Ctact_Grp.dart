...
class Ctact_Grp extends StatefulWidget {
  final VoidCallback onMaj;
  const Ctact_Grp({Key? key, required this.onMaj}) : super(key: key);


  @override
  State<Ctact_Grp> createState() => _Ctact_GrpState();
}

class _Ctact_GrpState extends State<Ctact_Grp> {
  ...
  Future initLib() async {
    ...
    await DbTools.getContactType(DbTools.gClient.ClientId, DbTools.gGroupe.GroupeId, "GRP");
    Filtre();
    AlimSaisie();
  }
  ...
  void ToolsBarAdd() async {
    print("ToolsBarAdd");
    await DbTools.addContactAdrType(DbTools.gClient.ClientId, DbTools.gGroupe.GroupeId, "GRP");
    await DbTools.getContactGrp(DbTools.gClient.ClientId, DbTools.gGroupe.GroupeId);
  ...
  Widget ContactGridWidget() {
    ...
    return new EasyTableTheme(
        child: new EasyTable<Contact>(visibleRowsCount: 18, _model, onRowTap: (Contact) async {

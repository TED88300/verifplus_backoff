...
//      print("DbTools ${element.Param_Gamme_Type_Organe}");
    ...
    //   _model = EasyTableModel<Param_Gamme>(rows: DbTools.ListParam_Gammesearchresult, columns: wColumns);
...
//   await LoadCSV();
  ...
  Widget Param_GammeGridWidget() {
    List<EasyTableColumn<Param_Gamme>> wColumns = [
      new EasyTableColumn(
          name: 'Id',
          grow: 1,
          stringValue: (row) => "${row.Param_GammeId} ${row.Param_Gamme_Ordre}"
              ""),
      new EasyTableColumn(name: 'Fabriquant', grow: 18, stringValue: (row) => "${row.Param_Gamme_FAB_Lib}"),
      new EasyTableColumn(name: 'Presssion', grow: 1, stringValue: (row) => "${row.Param_Gamme_PRS_Lib}"),
      new EasyTableColumn(name: 'Classes', grow: 1, stringValue: (row) => "${row.Param_Gamme_CLF_Lib}"),
      new EasyTableColumn(name: 'Maniabilite', grow: 18, stringValue: (row) => "${row.Param_Gamme_MOB_Lib}"),
      new EasyTableColumn(name: 'Modèle', grow: 18, stringValue: (row) => "${row.Param_Gamme_PDT_Lib}"),
      new EasyTableColumn(name: 'Charge', grow: 1, stringValue: (row) => "${row.Param_Gamme_POIDS_Lib}"),
      new EasyTableColumn(name: 'Gamme', grow: 18, stringValue: (row) => "${row.Param_Gamme_GAM_Lib}"),
      new EasyTableColumn(name: 'Ref', grow: 1, stringValue: (row) => "${row.Param_Gamme_REF}"),
    ];

    print("Param_GammeGridWidget");
    EasyTableModel<Param_Gamme>? _model;

    _model = EasyTableModel<Param_Gamme>(rows: DbTools.ListParam_Gammesearchresult, columns: wColumns);

    return new EasyTableTheme(
        child: new EasyTable<Param_Gamme>(
          _model,
          visibleRowsCount: 18,
...
//**********************************
  ...
  Widget _buildDelete(BuildContext context, RowData<Param_Gamme> rowData) {
    return IconButton(
      icon: Icon(
        Icons.delete,
        color: Colors.red,
      ...
      onPressed: () async {
        DbTools.delParam_Gamme(rowData.row);
        await Reload();
      },
    );
  }
